...
class CalFileParser {
    ...
    private $DTfields = array('start', 'end');
            ...
            foreach ($event_key_pairs as $line) {
                ...
                if ($line[0] == ' ') {
                    ...
                    if ($key == "dtstart;tzid=europe/paris")
                        $key = "start";
                    if ($key == "dtend;tzid=europe/paris")
                        $key = "end";
                    if ($key == "summary")
                        $key = "title";
                        
                    if (in_array($key, array("attendee;role=chair","attendee;role=req-participant","dtstamp")))
                        continue;
                    ...
                    if (in_array($key, $this->DTfields)) {
                        ...
                        $date = DateTime::createFromFormat('Ymd-His', $dt_str, new DateTimeZone('Europe/Paris'));
                        if ($date !== false) {
